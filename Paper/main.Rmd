...
rating_and_parameters_long_8b <- merge(data_list$rating, get_theta(exp2_trait_model_8b)) |>
  within({
    d <- ifelse(us_valence == "positive", d_positive, d_negative)
    C <- ifelse(us_valence == "positive", C_positive, C_negative)
  })

rating_and_parameters_long <- merge(data_list$rating, get_theta(exp2_trait_model))
