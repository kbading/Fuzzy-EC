...
We found one participants who gave the same response on all trials of the evaluation task (but different responses across trials in the memory task).
...
Parameter estimates and confidence intervals are reported in *Figure xxx* (Panel A).
...
```{r}
# model_exp1 <- lm(ec_effect ~ cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a),data = ratings_and_parameters)
# summary(model_exp1)
# ap_model_exp1 <- apa_print(model_exp1)
# ap_model_exp1
```

```{r}
# model_exp2 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
...
```{r}
# model_exp3 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
...
```{r}
# cor.test(ratings_and_parameters$C,ratings_and_parameters$d)
```
