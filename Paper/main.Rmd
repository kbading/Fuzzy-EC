...
```{r}
exp1_trait_model <- readRDS(study_folders$wsw1, "model-objects", "trait-mpt.rds")
# exp1_trait_model <- readRDS(study_folders$wsw1, "model-objects", "trait-mpt-wsw-8b.rds")
```
...
```{r}
exp2_trait_model <- readRDS(study_folders$wsw2_main, "model-objects", "trait-mpt.rds")
# exp2_trait_model <- readRDS(study_folders$wsw2_main, "model-objects", "trait-mpt-wsw-8b.rds")
```
...
We recruited xxx participants (xxx$\%$ female; $M_{age} = xxx$; $SD_{age} = xxx$).
As pre-registered, we excluded xxx participants who failed at least one control measure (by selecting at least one physical activity, by reporting a lack of attention or by responding with "I have just clicked through, please discard my data").
Taken together, this resulted in a final sample size of xxx participants (xxx in the valence focus condition and xxx in the age focus condition condition).
...
```{r}
exp3_trait_model <- readRDS(study_folders$wsw3_main, "model-objects", "trait-mpt.rds")
# exp3_trait_model <- readRDS(study_folders$wsw3_main, "model-objects", "trait-mpt-wsw-8b.rds")
```
