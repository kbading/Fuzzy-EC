...
Participants rated how positive or negative they found each face on a 11-point Likert scale ranging from "very negative" (-5) to "very positive" (+5).
The 48 trials were separated by blank screens presented for 500 ms.
Trial order was randomized for each participant anew.
...
exp1_ratings_anova <- aov_ez(data = subset(exp1_ratings,us_valence %in% c("positive","negative")), id = "sid", dv = "evaluative_rating", within = c("us_valence","cs_sex"))

exp1_ec <- t.test(exp1_ratings_wide$positive,exp1_ratings_wide$negative,paired=TRUE,alternative="greater")
...
hypothesis_restrictions_8b <- list(
  ...
  , a_eq.5 = list(a = .5)
  , b_eq.5 = list(b = .5)
  , C_eq  = list(C_negative = C_positive)
  , d_eq  = list(d_negative = d_positive)
)
...
models_8b <- lapply(
  ...
  , FUN = function(x) {
    fit_mpt(
      model = file.path(study_folders$wsw1, "wsw-8b.eqn")
...
```{r}
# model_exp1 <- lm(ec_effect ~ cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a),data = subset(ratings_and_parameters,sid!=43))
#summary(model_exp1)
...
```{r}
# model_exp1 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (us_valence|sid)
