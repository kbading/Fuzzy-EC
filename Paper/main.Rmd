...
As in the previous experiments, we explored the relationships between MPT parameters and individual EC effects, using a joint-modeling approach implemented in *Stan* [@carpenter_stan_2016].
...
We found strong support for non-zero $D$ and $C$ parameters in both task focus conditions, for a non-zero $d$ parameter in the "valence focus" condition, and for departure from $.5$ for the $b$ parameter in the "age focus" condition, all $BF_{10}>1,000$.
...
## Discussion 
In the present experiment, we replicated Gast and Rothermund's (2011) Experiment 2, comparing EC effects and CS-US pairing memory between participants who performed different judgment tasks during the learning phase.
Replicating Gast and Rothermund's (2011) task focus effect on evaluative conditioning, we found that EC effects were substantial only among participants in the "valence focus" condition and not among participants in the "age focus" condition.
Using the newly proposed MPT model, we also replicated Gast and Rothermund's (2011) task focus effect on US valence memory: the $d$ parameter (measuring US valence memory in the absence of US identity memory) was substantially larger in the "valence focus" condition than in the "age focus" condition.
Going beyond the findings by Gast and Rothermund (2011), the MPT model also delivered strong evidence for the absence of US valence memory in the "age focus" condition (as indicated by the Bayes Factor for the test of $d_{age}$ against zero).
Moreover, and again in line with the original findings, we also found strong support that the task focus effect on EC was mediated by US valence memory (while no such support was found for a mediation by US identity or CS recognition memory).
Taken together, these findings provide strong support for the "valence focus" explanation of the $d$ parameter and of evaluative conditioning: US valence memory (without US identity memory) and conditioned CS evaluations are developed when the CS-US pairings are processed in terms of valence instead of in terms of other (non-evaluative) dimensions.
...
Since we used the same materials (CSs and USs) in both conditions, these parameter differences can only be explained by the differences between the two judgment tasks.
In line with prior speculations (see introduction), the present experiment thus demonstrates that different learning conditions may not only affect CS-US pairing memory but also CS recognition memory, underlining the need for separate and non-confounded measures of the different forms of memory that are involved in EC effects.
Moreover, the joint direction of the task focus effects on $D$ and $C$ parameters points towards a possible explanation of these effects: given that both parameters were substantially lower in the "age focus" condition, it seems likely that the non-evaluative judgment task (focused on the age typicality of the CS-US pairs) was more difficult than the evaluative judgment task (resulting in less mental capacity for memorizing the details of the presented stimuli and, by implication, lower levels of CS recognition and US identity memory).

Before we discuss the broader implications of the present findings (and those from the previous experiments), we want to address the inconsistent effects of task focus on the $b$ parameter.
...
We assumed that a stronger tendency towards guessing "old" may emerge as a compensation for lower levels of CS recognition memory (to assure that a plausible number of stimuli is classified as "old") and therefore expected to find a larger $b$ parameter in the "age focus" condition (than in the "valence focus" condition) also in the main experiment (in line with our expectation of a smaller $D$ parameter in the "age focus" condition than in the "valence focus" condition).
Against this background, we were somewhat surprised by the direction of the $b$ parameter difference ($b_{valence}>b_{age}$) obtained in the present experiment.
Based on the fact that we used the same experimental software (including materials, instructions and procedures) in both studies, we considered the possibility that the $b$ parameter differences (obtained in the pilot study and in the present experiment) reflected statistical artifacts resulting from the aggregation of response frequencies across participants.
...
Based on these findings (and for lack of a better explanation), we assume that the (inconsistent) $b$ parameter differences (in the MPT model analyses on aggregated response frequencies) constitute statistical artefacts that disappear when the MPT parameters are estimated at the level of individual participants.
