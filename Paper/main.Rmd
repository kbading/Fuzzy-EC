...
Memory for the US valence is usually measured in one of two ways: either by recoding responses from the US identity memory task (in terms of their valence) or by implementing a separate procedure in which US valence memory is probed directly.
...
The chance level in a typical measure of US valence memory is thus $.5$ (corresponding to the inverse of the two US valences represented in the measure) and therefore higher than that in typical measures of US identity memory (assuming that more than two response options are presented).
...
Affect-as-information happens when participants cannot remember the valence of the previously paired US and then rely on their affective reaction towards the CS to select a response in the US valence memory task.
...
As long as guessing processes are uncorrelated across measures (CS evaluation, US identity memory, US valence memory), such a sample will produce a strong (non-perfect) correlation between the two memory measures as well as comparably strong (non-perfect) correlations between either memory measure and the measure of CS evaluation.
Importantly, the multicollinearity between the two memory measures will make it statistically impossible to reveal the underlying relationship between EC effects and US identity memory (e.g., in a linear regression predicting CS evaluation from US valence and the two memory measures).

When guessing is performed independently across measures, the inferability of US valence from US identity will thus undermine evidence for the role of US identity memory in the emergence of EC.
Matters are worse, however, when guessed responses are in fact correlated across measures (e.g., due to affect-as-information): in this case, evidence for the relationship between EC effects and US identity memory will be nullified and replaced by evidence for a non-existing relationship between EC effects of US valence memory.
To illustrate this point, imagine the same sample of participants now using affect-as-information to generate responses in the CS evaluation and memory measures whenever they cannot remember the previously paired US.
...
hypothesis_restrictions <- list(
  ...
  , Ceval0   = list(C_x1 = .Machine$double.eps)
  , Cmem0   = list(C_x2 = .Machine$double.eps)
  , deval0   = list(d_x1 = .Machine$double.eps)
  , dmem0   = list(d_x2 = .Machine$double.eps)
...
```{r}
model1 <- stats::lm(ec_effect ~ (cntr(D) + cntr(C) + cntr(d) + cntr(b) + cntr(a))+task_order, data = ratings_and_parameters)
summary(model1)
```
...
```{r}
model5_exp2 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + task_order + us_valence:task_order + (1|sid)
           , data = subset(ratings_and_parameters_8b_long,us_valence%in%c("positive","negative")))
...
```{r}
model5_exp2 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
           , data = subset(ratings_and_parameters_8b_long,us_valence%in%c("positive","negative")))
...
```{r}
ggeffect(model5_exp2, terms = ~ b) |> plot()
```
