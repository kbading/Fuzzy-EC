...
Despite their many differences, most of these accounts share a core idea: one way or another, they all view memory for the CS-US pairings as the cognitive foundation of EC.
...
In early research, pairing memory was conceptualized as associative memory for the specific US that had been paired with a given CS [e.g., @baeyens_contingency_1990; Walther & Nagengast, 2006].
...
```{r bla, out.width="100%"}
knitr::include_graphics(file.path(study_folders$paper,"mpt_wsw_model_exp3_KB2.pdf"))
```

```{r bla2, out.width="100%"}
knitr::include_graphics(file.path(study_folders$paper,"mpt_wsw_model_exp3_KB2bb.pdf"))
```
...
Based on these structural similarities, we considered the possibility that the problems involved in the measurement of CS-US pairing memory can be overcome with the same tool that solves the problems connected to the measurement of social categorization (for further details, see Klauer & Wegener, 1998).
...
Whenever participants select the "old" response (for a given CS or distractor),
...
Note that, on each trial the US memory task, positive and negative USs are shown in equal numbers (including the correct US whenever participants select the "old" response for an actual CS).
...
Moreover, US valence memory is measured by the $d$ parameter which represents the conditional probability to remember the correct US valence given that (a) the CS was recognized as old and (b) the previously paired US could not be remembered.
...
The second shortcoming (that US valence may be used to guess US identity) is solved by the $g$ parameter, which represents the conditional probability of guessing the correct US when the US valence can be remembered.
...
By estimating this parameter, the MPT model avoids bias from response tendencies towards positive (or negative) response options, thereby overcoming the third of the previously discussed shortcomings.
...
<!-- As indicated by different subscripts (e.g., $D_{pos}$ vs. $D_{neg}$), the MPT parameters may in principle take different numerical values in each of the three model trees, representing the idea that cognitive states may have different probabilities for different stimulus types. -->
