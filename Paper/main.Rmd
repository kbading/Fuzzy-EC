...
The first three sentences were long and convoluted (including general statements about attitude research).
...
Moreover, the mean EC effects in the two task focus conditions were tested against zero using one-sided t-tests ($\alpha=.05$).
...
Next, the frequency distribution of the joint memory index (aggregated across participants)
was analyzed with a Bayesian implementation of the MPT model, which is available in
the R packages TreeBUGS (Heck, Arnold, & Arnold, 2018) and MPTmultiverse (Singmann,
Heck, Barth, & Aust, 2020; see also Singmann et al., 2024). 
For each MPT parameter and, we used (default) Beta priors with α = β = 1, resulting in a uniform prior distribution in the unit interval. 
