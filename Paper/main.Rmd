...
Importantly, Hütter et al. (2012) also presented simulated data showing that either type of affect-as-information (based on pre-existing vs. conditioned attitudes) leads to artifactual correlations between US valence memory and EC effects, making empirical demonstrations of EC without CS-US pairing memory highly difficult.
...
However, since the use of single-valence US identity tasks does not resolve the first shortcoming (inferring US valence from US identity), the statistical relationships between US identity memory and EC effects will still be nullified in linear regression analyses that include both memory measures as predictors.
...
Most importantly, @klauer_who_1998 proposed a Multinomial Processing Tree (MPT) model that disentangles several confounds in the standard measure of social categorization in the "Who said what?" paradigm (Taylor, Fiske, Etcoff, & Ruderman, 1978).
...
To apply the MPT model in EC research, we thus used constrained model variants including five or seven freely estimated parameters (for a given between-subjects condition).
...
Despite being more restrictive, the 5-parameter variant may thus be preferable to the 7-parameter model in empirical applications (given that the two model variants show similar ability to explain response data from the two-step memory task, which is an open empirical question).
...
Moreover, we also found that CS recognition, US identity and US valence memory were all boosted by the evaluative judgment task, but only US valence memory was found to mediate the concomitant increase in EC effects.
Taken together, these results provide evidence for the suitability and power of the MPT model in explaining theoretically relevant moderations of EC.
...
To increase similarity with the "Who said what?" paradigm, we used images of human faces as CSs.
As in previous EC studies [e.g., @stahl_subliminal_2016], the faces displayed neutral expressions and were paired with clearly valenced images (positive vs. negative) serving as USs.
...
Due to its exploratory nature, Experiment 1 was not preregistered.
...
This expectation was based on prior research showing above-chance performance in a US identity measure that included only USs of the correct valence (Stahl & Bading, 2020).
...
As a result, previous indications of above-chance performance on these measures provide only limited grounds for expecting US valence memory in the absence of US identity memory (as measured by the $d$ parameter).
...
Participants were recruited through Prolific and received £2.25 for their participation (estimated study duration: 15 minutes).

The sampling pool was restricted to English speakers with at least 100 previous submissions and an approval rating of at least 90%. Sex was balanced. 
...
The CS pool comprised 48 black and white images of human faces with neutral expressions (24 female faces, 24 male faces).
The face images have been used as CSs in previous EC research (e.g., Waroquier, Abadie, & Dienes, 2020).
...
The positive and negative images differed with regard to valence, Welch’s $t(20.23) = 33.36, p < .001$, but did not differ significantly regarding arousal, Welch’s $t(21.96) = 0.82, p = .419$. 
...
In the test phase, participants performed a two-step memory task and then a CS evaluation task. 
...
Participants rated how positive or negative they found each face on a 11-point rating scale ranging from "very negative" (-5) to "very positive" (+5). Responses were stored as numerical values ranging from 0 to 11. 
The 48 trials were separated by blank screens presented for 500 ms.
Trial order was randomized for each participant anew.
...
After the test phase, participants were asked whether they paid attention to the images presented throughout the experiment.
...
In these two questions, participants were told that their responses would not affect their payment.
...
After pressing the spacebar, participants were redirected to Prolific and compensated for their participation.
...
using point estimates as predictor values ignores the uncertainty associated with the estimation of each MPT parameter. Ignoring this uncertainty follows from the assumption of linear regression analysis that predictor values are free of measurement error [cf., @klauer_unbiased_1998]. However, this assumption is unlikely to hold. 
<!--thereby violating the assumption of linear regression analysis that predictor values are free of measurement error 
JB: I was unsure how this wording, as it was as if treating MPT parameter estimates as free of measurement error was in itself a violation of the assumption. Because this is not the case (it exactly follows from the the assumption), I tried to revise this part but I am unsure this is clear enough. --> 
As a result, using point estimates to predict EC effects may lead to biased conclusions about the statistical relationships between MPT parameters and individual EC effects.
...
The mean difference (i.e., the mean EC effect) was small in size, $d_{Cohen}= 0.26$, and reached significance only in a one-tailed test, `r exp1_posneg$full_result`.
...
The $D$ parameter estimate was significantly larger than zero, `r apa_wsw_comps$full_result$D_eq0`. This indicates that participants recognized CSs as old and detected distractor images as new in $45.6$% of trials.
This implies that participants did not recognize CSs as old, and did not detect distractor stimuli as new, in $1-D = 54.4$% of trials.
...
The $d$ parameter estimate was significantly larger than zero, `r apa_wsw_comps$full_result$d_eq0`. This estimate indicates that, in the subset of recognized CSs without US identity memory, participants possessed US valence memory in approximately $19.9$% of trials.
...
## Discussion
In Experiment 1, we used the newly adapted MPT model we presented in the introduction to estimate EC-related memory in the context of an EC procedure augmented with a memory task.
...
As expected, the significant $D$ parameter indicated that participants did correctly recognize the CS as "old" and the distractor images as "new." The significant $C$ parameter indicated that participant correctly retrieved US identity from memory for a subset of CSs.
Moreover, the significant $d$ parameter showed that participants occasionally retrieved US valence memory without retrieving US identity information for a subset of recognized CSs.
Finding an above-0 $d$ parameter is a clear indication that US valence memory cannot be reduced to inferences based on US identity memory. Provided the shortcomings of previous US valence measures discussed in the introduction, the present result is, to our knowledge, new and strongest (than previous research) empirical evidence supporting the view that individuals may sometimes retrieve from memory the valence of a US a CS was previously paired with. 
<!-- finding is the first unambiguous demonstration that US valence memory constitutes an independent form of CS-US pairing memory.-->
In line with previous research [e.g., @stahl_respective_2009-1], the results of Experiment 1 also tentatively suggested that EC effects are positively related to US valence memory (as measured by the $d$ parameter) but unrelated to US identity memory (as measured by the $C$ parameter).
...
To try to boost EC effects, we used pronounceable non-words as CSs (which might be more neutral and thus easier to condition than the human faces used in the first experiment).
...
Moreover, we also manipulated the order in which memory and CS evaluation tasks were performed in the test phase.
This task order manipulation was implemented to determine whether administering the two-step memory task first influences the EC effect.
...
## Method
Experiment 2 was preregistered on the Open Science Framework (OSF).
The preregistration, materials, and data are publicly available at: https://osf.io/rqkvy/.
The data collection was conducted online.
...
Participants were recruited through Prolific and received £2.25 for their participation (estimated study duration: 15 minutes).
As in the Experiment 1, the sampling pool was restricted to English speakers who had not participated in our previous EC studies with at least 100 previous submissions and an approval rating of at least 90%. Sex was balanced.
...
To avoid a final sample size smaller than $N = 156$ after applying the preregistered exclusion criteria, we chose to recruit 172 participants (the required sample size increased by 10%).

### Materials
The experiment was programmed with *lab.js* [@henninger_lab_2021] and exported to an HTTPS-protected website with JATOS [@lange_just_2015].

The CS pool comprised 54 non-words made up of five to seven letters (e.g., *botsy*, *ikzunt*, *ampfong*; see the online study program for the complete list of non-words).
...
As USs, we used 24 OASIS pictures. Twenty-two (out of 24) of these pictures were identical to the ones we used in Experiment 1; 2 negative pictures were replaced by two other, less ambiguous images (see the online study program for the complete US pool).
...
The task instructions were largely identical to those used in the first experiment but differed slightly between task order conditions (for details, see preregistration).
...
##### Memory task
In the two-step memory task, the 24 CSs and the 24 distractor stimuli were displayed individually and without time limit.
...
#### Control measures and debriefing
Control measures and debrefing were highly similar to Experiment 1. After the test phase, participants were asked whether they paid attention to the non-words and images presented throughout the experiment.
The response options ("Yes" vs. "No") were presented as two buttons.
Subsequently, participants were asked whether they took the requested responses seriously.
The response options ("I have taken the requested responses seriously" vs. "I have just clicked through, please discard my data") were again presented as two buttons.
In these two questions, participants were told that their responses would not affect their payment.
...
The responses from the two-step memory task were recoded into a joint memory index according to the same scheme as used in Experiment 1 (see above).
...
As preregistered, we then tested all seven parameters against their reference value ($0$ for $D$, $C$ and $d$ parameters; $.5$ for $a$ and $b$ parameters). 
...
Because the memory data was well described by the 5-parameter model (`r fit$model5p$T1`; `r fit$model5p$T2`), we decided to use this simpler model variant to conduct the preregistered regression analyses aimed at predicting individual EC effects from participant-level parameter estimates.
...
In yet another departure from the preregistered analyses, we used the joint modeling approach introduced in Experiment 1 (see above) instead of the preregistered (but less appropriate) two-step procedure.
...
However, to simplify the results section, we decided to report only the main effects model that was mentioned in the preregistration (which, in addition to being simpler, also produced the most straightforward results).
...
As in Experiment 1, we used the probabilistic programming language *Stan* [@carpenter_stan_2016] to implement the joint-modeling approach.
...
The between-subjects ANOVA showed a significant intercept, `r exp2_ec_anova$full_result$Intercept`, and a non-significant main effect of task order, `r exp2_ec_anova$full_result$task_order`.
The significant intercept indicated a robust EC effect across task order conditions (`r mean_ec`, `r sd_ec`).
...
This result shows that, as in Experiment 1, participants showed a bias towards guessing "new" whenever they did not recognize a CS as old (or detect a distractor as new).
Turning to the $d$ parameters, the present results differed from those obtained in the first experiment.
...
For the remaining MPT parameters, the posterior differences between group mean were negligible.
...
Figure\ \@ref(fig:exp2-regression) shows the relationships between participant-level MPT parameter estimates and marginal EC effects.
...
By contrast, the significant $d$ parameter we found in Experiment 1 was not replicated in the present experiment.
As a result, at the group level, Experiment 2 did not provide robust evidence for US valence memory in the absence of US identity memory.
However, this result should be qualified, as we found strong support for a positive relationship between participant-level $d$ parameter estimates and individual EC effects.
...
Based on the present findings, we can only speculate why the majority of participants did not demonstrate much US valence memory (in the subset of CSs without US identity memory).
A possible explanation comes from previous research conducted by Gast and Rothermund (2011).
As explained in the introduction, Gast and Rothermund (2011) showed that EC effects and CS-US pairing memory were boosted by a valence focus when exposed to the pairings. Such a valence focus can be induced through an evaluative judgment task performed during the learning phase.
In the present experiment, participants did not perform such a judgment task nor were they otherwise told that stimulus valence was a relevant factor (e.g., by mentioning valence in the instructions presented prior to the learning phase).
As a consequence, a majority of participants may have observed the CS-US pairings without processing the USs in terms of their valence. If so, it follows that we found low levels of US valence memory and small EC effects overall. A minority of participants may have processed the pairings under a spontaneous valence focus, thus developing higher levels of US valence memory and showing larger EC effects.
To test this "valence focus" explanation and test our focal hypotheses again, we conducted Experiment 3.
...
First, we wanted to replicate Gast and Rothermund's (2011) valence focus effect on evaluative conditioning (i.e., a larger EC effect for participants instructed to focus on valence vs. on another dimension).
...
Second, we sought to replicate Gast and Rothermund's (2011) valence focus effects on US identity and US valence memory, using the newly introduced MPT model.
In Gast and Rothermund's (2011) Experiments 1 and 3, US identity memory was measured with a standard US identity task showing a mixed set of response options (50% positive USs and 50% negative USs). Gast and Rothermund measured US valence memory by recoding the selected USs (from the US identity task) in terms of their valence.
...
In addition to disentangling (potential) changes in CS recognition and CS-US pairing memory, the present MPT modeling approach also allowed us to test whether retrieving US valence information from memory in the absence of US identity retrieval requires a focus on valence while observing the CS-US pairings.
...
Third, we sought to re-examine the roles of US identity and US valence memory in the mediation of the valence focus effect on evaluative conditioning.
...
However, as explained earlier, multiple regression analyses (and, by implication, multiple mediation analyses) that include non-independent measures of US identity and US valence memory as competing predictors (of individual EC effects) cannot produce strong evidence for (against) a relationship between US valence memory (US identity memory) and evaluative conditioning.
In the present experiment, these limitations were overcome by conducting a multiple mediation analysis with participant-level MPT parameters as competing predictors of individual EC effects.
By including independent measures of CS recognition, US identity and US valence memory as potential mediators (while controlling for response tendencies in the underlying measurement task), we sought to clarify the valence focus effect reported by Gast and Rothermund (2011).
...
## Method
The experiment was preregistered on the OSF.
The preregistration, materials, and data are publicly available at: https://osf.io/rqkvy/.
The data collection was conducted online.
...
Participants were recruited through Prolific and received £3.00 for their participation (estimated study duration: 20 minutes).
...
As before, Prolific users who had participated in previous pretests and experiments were excluded from the sampling pool. Sex was balanced.
...
The number of recruited participants was based on a preregistered sampling plan (see OSF repository).
We preregistered a minimum sample size (per task focus condition), a maximum sample size (across task focus conditions), and two stopping criteria (determining the [dis-]continuation of the data collection within the preregistered sample size range).
...
Based on the available data, we fitted the preregistered 10-parameter model (estimating joint $D$, $C$, $d$, $a$ and $b$ parameters separately for the two task focus conditions).
...
After a maximum of 1,000 ms, the CS-US pair and the three hyphens were replaced by an empty screen.
...
#### Test phase
After the learning phase, participants entered the test phase. 
In the test phase, participants performed two  tasks: the CS evaluation task followed by the two-step memory task. 
...
Subsequently, participants were asked whether they paid attention to the non-words and images presented throughout the experiment, and whether they took the requested responses seriously. These questions were identical to the ones used in Experiments 1-2.
...
As predicted, the mean EC effect was larger in the "valence focus" condition than in the "age focus" condition (see below); we therefore conducted a one-sided test.
Moreover, the mean EC effects in the two task focus conditions were tested against zero using one-sample t-tests ($\alpha=.05$).
In the "valence focus" condition, we performed a one-sided test because the mean EC effect was larger than zero (as predicted).
...
#### Memory data
As before, we implemented the preregistered analyses without exceptions.
...
We then tested each MPT parameter against its reference value ($0$ for $D$, $C$ and $d$ parameters; $.5$ for $a$ and $b$ parameters).
To this aim, we fitted ten 9-parameter models each of which included a different parameter restriction (e.g., $D_{valence}=0$)
Each of these nested models was then compared to the unrestricted 10-parameter model, using Bayes Factors.
Next, we tested the effect of task focus on each MPT parameter ($D$, $C$, $d$, $a$ and $b$).
To this aim, we fitted another five 9-parameter models each of which included a different parameter restriction: (1) $D_{valence}=D_{age}$, (2) $C_{valence}=C_{age}$, (3) $d_{valence}=d_{age}$, (4) $a_{valence}=a_{age}$, and (5) $b_{valence}=b_{age}$ (the subscripts indicate the task instruction condition: *valence* for the valence focus condition; *age* for the age focus condition).
As before, these nested models were compared to the unrestricted 10-parameter model, using Bayes Factors.
...
As in Experiments 1-2, we explored the relationships between MPT parameters and individual EC effects, using a joint-modeling approach implemented in *Stan* [@carpenter_stan_2016].
...
To explore the mediation of the valence focus effect on evaluative conditioning, we also calculated Bayes Factors for the direct effect of task focus as well as for its indirect effects through the five MPT parameters ($D$, $C$, $d$, $a$ and $b$), using the independent-paths method proposed by @liu_bayesian_2023.
...
The mean EC effect in the "valence focus" condition was significantly larger than zero, `r t.val$full_result`, while the mean EC effect in the "age focus" condition did not significantly differ from zero, `r t.age$full_result`.
...
Parameter estimates and credible intervals from the unrestricted 10-parameter model are reported in Table\ \@ref(tab:exp3-param).
We found strong support for above-zero $D$ and $C$ parameters in both task focus conditions, for an above-zero $d$ parameter in the "valence focus" condition, and for departure from $.5$ for the $b$ parameter in the "age focus" condition, all $BF_{10}>1,000$.
Moreover, we found strong support against departure from zero for the $d$ parameter in the "age focus" condition, $BF_{10}=0.06$, and against departure from $.5$ for the $a$ parameter in both focus conditions, both $BF_{10}\leq0.08$.
Finally, we found moderate support against departure from $.5$ for the $b$ parameter in the "valence focus" condition, $BF_{10}=0.17$.
...
For participant-level $a$ and $C$ parameter estimates, the evidence for and against a relationship with individual EC effects was inconclusive ($C$ parameter: `r apa_model_lm$full_result$C`; $a$ parameter: `r apa_model_lm$full_result$a`) .
...
In the multiple mediation analysis, we found very strong support for an indirect effect of task focus on EC through the $d$ parameter, `r indirect_effects$d`, and moderate support against such an indirect effect through the $b$ parameter, `r indirect_effects$b`.
...
Replicating Gast and Rothermund's (2011) task focus effect on evaluative conditioning, we found that the EC effect was significant among participants in the "valence focus" condition, but not among participants in the "age focus" condition.
Using the newly proposed MPT model, we also replicated Gast and Rothermund's task focus effect on US valence memory: the $d$ parameter (measuring US valence memory in the absence of US identity memory) was substantially larger in the "valence focus" condition than in the "age focus" condition.
Importantly, going beyond the findings reported by Gast and Rothermund, the MPT model also delivered strong evidence for the absence of US valence memory in the "age focus" condition (as indicated by the Bayes Factor for the test of $d_{age}$ against zero).
Moreover, and again in line with the original findings, we also found strong support that the task focus effect on EC was mediated by US valence memory. We found no such support for a mediation by US identity or CS recognition memory.
Taken together, the present findings provide strong support for the "valence focus" explanation of the $d$ parameter and of evaluative conditioning: US valence memory (without US identity memory) and conditioned CS evaluations are developed when the CS-US pairings are processed in terms of valence instead of in terms of other (non-evaluative) dimensions.

The present experiment also revealed substantially larger $D$ and $C$ parameters in the "valence focus" condition than in the "age focus" condition.
Since we used the same materials (CSs and USs) in both conditions, we think these parameter differences should be explained by the differences between the two judgment tasks.
In line with prior speculations (see introduction), the present experiment thus demonstrates that different learning conditions may not only affect different aspects of CS-US pairing memory but also CS recognition memory, underlining the need for separate and non-confounded measures of the different forms of memory that are involved in EC effects.
Moreover, $D$ and $C$ parameters estimates were lower in the "age focus" than in the "valence focus" condition. A possible explanation of this result is that the non-evaluative judgment task (focused on the age typicality of the CS-US pairs) was more difficult than the evaluative judgment task (resulting in less mental capacity for memorizing the details of the presented stimuli and, by implication, lower levels of CS recognition and US identity memory). Another possibility is that the two tasks were not equated in terms of processing depth. While both tasks imply to process the meaning of the pairings, processing valence may involve deeper processing (more relevant for the self; echoing personal preferences and past experiences). If so, higher memory retrieval (whether for recognition memory or US identity memory) could be a level-of-processing effect (Craik, 2002; Craik & Lockhart, 1972). These possibilities could be tested in future research. 

Before we turn to discussing the broader implications of the present experiments in the General Discussion, we want to address the inconsistent effects of task focus on the $b$ parameter (i.e., a bias towards "old" responses without recognition memory).
As explained above, the pilot study (in which we tested the present materials and procedures) showed a larger $b$ parameter in the "age focus" condition than in the "valence focus" condition.
We assumed that a stronger tendency towards guessing "old" may emerge as a compensation for lower levels of CS recognition memory. This is because participants may want to ensure that a plausible number of stimuli is classified as "old." As a result, as we expected recognition memory to be lower when processing age rather than valence, we also expected to find a larger $b$ parameter in the former than in the later.
Against this rationale, we were somewhat surprised by the direction of the $b$ parameter difference ($b_{valence}>b_{age}$) obtained in the present experiment.
Because we used the same experimental software (including materials, instructions and procedures) in both data collections, we considered the possibility that the $b$ parameter differences (obtained in the pilot study and in the present experiment) reflected statistical artifacts resulting from the aggregation of response frequencies across participants.
To probe this possibility, we implemented separate joint models for the two studies and checked the task focus effect on the $b$ parameters: in line with the "aggregation artifact" explanation, these participant-level MPT analyses did not provide support for substantial $b$ parameter differences across task focus conditions.
Based on these findings (and for lack of a better explanation), we assume that the (inconsistent) $b$ parameter differences (in the MPT model analyses on aggregated response frequencies) are statistical artefacts that disappear when the MPT parameters are estimated at the level of individual participants. This tentative artifact-based explanation awaits dedicated tests.

# General discussion

Assessing memories individuals retrieve and use when evaluating stimuli is crucial to advancing our understanding of the mechanisms, processes, and representations involved in the Evaluative Conditioning (EC) effect. While all EC theories do not explicitly give memory a major role, the EC effect involves retrieving "something" about the CS-US parings from the learning episode when presented with the to-be-evaluated CS alone (i.e., item-item associative memory, whether retrieval is conscious or not, for instance). Some accounts of the EC effect (e.g., Stahl & Aust, 2018; Gast, 2018) distinguish between US valence memory and US identity memory and assume US valence is enough for the EC effect to emerge. 

Past research supports the view the critical to-be-retrieved information is US valence rather than US identity (e.g., Stahl et al., 2009). However, as we noted in the introduction, evidence for this conclusion relies on measurement approaches that suffer from several shortcomings, mainly that past US valence and US identity measures are not independent, do not distinguish CS recognition memory from CS-US pairing, and do not distinguish memory processes from guessing tendencies. 

To address current limitations in CS-US pairing memory measurement approaches, we adapted an existing measurement model (Klauer & Wegener, 1998) to assess (recognition and CS-US pairing) memory and guessing tendencies. This Multinomial Processing Tree (MPT) model assumes that individuals have a certain probability to retrieve the identity of US that were paired with correctly recognized CS have (estimated by the $C$ parameter, see Figure 1). Importantly, even in the absence of US identity memory ($1-C$), it could be the case that individuals sometimes retrieve only less detailed, valence information about the US (estimated by the $d$ parameter). Critically this model allows revisiting in stronger tests whether such US valence memory in the absence of identity memory (1) can be found, (2) predicts the EC effect, and (3) conceptually replicates previous results.

In three experiments (Experiments 2 and 3 were preregistered while Experiment 1 was not), we modified the commonly used EC procedure to test the fit of the newly developed MPT model and derive parameter estimates. After the learning phase, the test phase featured new (distractor) neutral stimuli and included a memory task in addition to the CS evaluation task. The memory task consisted of two steps: first, participants had to indicate if a given stimulus was part of the learning phase (i.e., CS recognition task); second, in case of an affirmative response, participants had to select the US that was paired with the CS (i.e., contingent CS-US recognition task). We used the model across varied study settings: the CS were faces in Experiments 1 and 3 and non-words in Experiment 2; the US were images in Experiments 1 and 2 and trait adjectives in Experiment 3; the memory task was administered before the CS evaluation task in Experiment 1, in the opposite order in Experiment 3, and the order was counterbalanced in Experiment 2. **[Here: mention another valuable contribution, i.e., testing relationships between MPT parameters and the EC effect in a single model?]**

Overall, **[summary of the findings -- one sentence could also be added about the main result/conclusion in the abstract]**.

In the remainder of the General Discussion, we elaborate on how the present results clarify the role of US valence and identity memory in the EC effect in relation to Stimulus-Stimulus vs. Stimulus-Response learning theorizations. We then point out current limitations of the current approach (participant-level analyses; limited inferences regarding causality; the issue of assessing CS evaluation and memory at different times) and suggest how it can be built upon to better understand retrieval processes involved in the evaluative conditioning effect. 
...
## Limitations of the present MPT modeling approach

- participant-level instead of item level

- inferences regarding causality remain limited (i.e., difficult to know whether retrieval precedes evaluations)

- US valence vs. identity memory: mute regarding specific memory processes and representations -- we only know the retrieved information differs in specificity, which is already useful but could be further developed

- question of demands artifacts? regarding the EC effect in general, or its moderation by valence focus

- independence between US valence and US identity memory not yet shown

## Future applications 

- possible applications: testing effects of US distribution on memory and EC effects (Sperlich & Unkelbach)

- using a logic similar to the one adopted in the Two-Button Sets (TBS) task to jointly ask about memory and evaluations at a single point in time (Stahl et al., 2024; Ingendahl et al., 2025) and use the present MPT model on the data

- Search for functional dissociations between US valence memory and US identity memory on the one hand (to lend support to the notion they are functionnally independent retrieved information) and between US valence and EC effect on the other hand (to try to falsify the hypothesis that the EC effect is based on US valence memory)
