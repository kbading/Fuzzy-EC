...
## Methods
The pilot was not pre-registered.
...
```{r}
ggeffect(model5_exp2, terms = ~ C:task_order) |> plot()
```

```{r}
summary(emtrends(model5_exp2,var= "C", specs= ~ task_order),infer=TRUE)
```

```{r}
ggeffect(model5_exp2, terms = ~ D:task_order) |> plot()
```

```{r}
summary(emtrends(model5_exp2,var= "D", specs= ~ task_order),infer=TRUE)
```
