...
## Methods
Experiment 1 was conducted as a pilot and therefore not pre-registered.
The materials and data are publicly available at: https://osf.io/rqkvy/.
The experiment was run online.
...
<!-- Taken together, this resulted in a final sample size of 49 participants. -->
...
participants were thanked for their participation and asked to pay close attention to all instructions and tasks.
...
On each trial, a face image (CS, center-left position) was presented together with a valenced image (US, center-right position).
...
Each trial began with with the recognition memory task: participants were asked whether the face had been part of the nonword-image pairs presented in the learning phase.
...
In this task, the face image was presented together with eight valenced images (all of which had been shown as USs during the learning phase).
...
After the last trial of the evaluation task, participants read the following instruction slide: "The third and final part of the experiment is now finished. Now we have a few short questions about your experience performing the study. Press spacebar to continue."
...
First, we inspected the data for indications of non-compliance (among participants who passed the two control measures).
We found one participant who gave the same response on all trials of the evaluation task (but different responses across trials in the memory task).
We interpreted the response behavior in the evaluation task as an indicator of non-compliance and decided to exclude the participant from all analyses.
...
Next, the frequency distribution of the joint memory index (aggregated across participants) was analyzed with different versions of the MPT model.
The MPT model analyses were carried out using the R package *HMMTreeC*.
...
#par(mar = c(5, 4, 5, 2), xpd = TRUE,mfrow=c(1,2))
par(mfrow=c(1,2))
plot(
  models_8b$baseline
    , factors = list(y=c("a"=""))
  , parameters = c(
    ...
    , "C_positive" = expression(italic(C_positive))
    , "C_negative" = expression(italic(C_negative))
    , "d_positive" = expression(italic(d_positive))
    , "d_negative" = expression(italic(d_positive))
    , "b" = expression(italic(b))
    , "a" = expression(italic(a))
  )
  , ylim = c(0, 1)
  , args_legend = list(x = 4, y = 1.4)
)
title("Panel A")
title(xlab = "Model parameter", ylab = "Estimate")
plot(
  models$baseline
    , factors = list(y=c("a"=""))
  , parameters = c(
...
The 7-parameter variant of the MPT model provided an adequate account of the data, `r apa_wsw8b$stat$modelfit`.
Parameter estimates and confidence intervals from the 7-parameter model are reported in *Figure xxx* (Panel A).
The difference between $C_{positive}$ and $C_{negative}$ was non-significant, `r apa_wsw8b_comps$statistic$C_eq`, as was the difference between $d_{positive}$ and $d_{negative}$, `r apa_wsw8b_comps$statistic$d_eq`.
By implication, the restrictive 5-parameter variant of the MPT model (with $C_{positive}=C_{negative}$ and $d_{positive}=d_{negative}$) fit the data well, `r apa_wsw6$statistic$modelfit`.

Parameter estimates and confidence intervals from the 5-parameter variant of the MPT model are reported in *Figure xxx* (Panel B).
...
This implies that US identity was absent for $32.6$% of CSs recognized as "old".

##### CSs without US identity memory
The $d$ parameter estimate was significantly larger than zero, `r apa_wsw_comps$full_result$d_eq0`, indicating that in the subset of CSs for which US identity memory was absent, participants possessed US valence memory in approx. $19.9$% of trials.
By implication, participants did not possess US valence memory in approx. $80.1$% of trials (pertaining to the subset of CSs for which US identity memory was absent).

###### CSs without US valence memory
For the subset of CSs for which US valence memory was also absent, participants guessed a positive US in approx. $42.7$% of trials, as indicated by the $a$ parameter estimate.
This implies that participants guessed a negative US in approx. $57.3$% of trials (pertaining to the subset of CSs for which US valence memory was absent).
Note that the $a$ parameter estimate was significantly lower than $.5$, `r apa_wsw_comps$full_result$a_eq_5`, demonstrating an overall tendency towards selecting a negative US.

#### CSs not recognized as "old"
In the subset of CSs that were not recognized as "old", participants guessed "old" in $14.8$% of trials, as indicated by the $b$ parameter estimate.
By implication, participants guessed "new" in $85.2$% of trials in which a CS was not recognized as "old".
Note that the $b$ parameter estimate was significantly lower than $.5$, `r apa_wsw_comps$full_result$b_eq_5`, demonstrating an overall tendency towards selecting the "new" response.

##### CSs guessed as "old"
For the subset of CSs guessed as "old", participants guessed a positive US in approx. $42.7$% of trials, as indicated by the $a$ parameter estimate.
This again implies that participants guessed a negative US in approx. $57.3$% of trials (pertaining to the subset of CSs that were guessed as "old").

#### Distractor stimuli not recognized as "new"
In the subset of distractor stimuli that were not recognized as "new", participants guessed "old" in $14.8$% of trials, as indicated by the $b$ parameter estimate.
By implication, participants guessed "new" in $85.2$% of trials in which a distractor stimuli was not recognized as "new".

##### Distractor stimuli guessed as "old"
As for CSs guessed as "old", participants guessed a positive US in approx. $42.7$% of trials, as indicated by the $a$ parameter estimate.
This again implies that participants guessed a negative US in approx. $57.3$% of trials (pertaining to the subset of distractor stimuli that were guessed as "old").
...
exp1_trait_model_8b_ec <- readRDS(file.path(study_folders$wsw1, "model-objects", "trait-mpt-wsw-8b-with-ec.rds"))
```

```{r}
#summary(exp1_trait_model)
# 
# plotParam(exp1_trait_model)
# 
#summary(exp1_trait_model_ec)
# 
# plotParam(exp1_trait_model_ec)
# 
#summary(exp1_trait_model_8b)
# 
# plotParam(exp1_trait_model_8b)
# 
#summary(exp1_trait_model_8b_ec)
# 
# plotParam(exp1_trait_model_8b_ec)
```
...
```{r}
model1_exp1 <- lm(ec_effect ~ cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a),data = ratings_and_parameters)
#summary(model1_exp1)
ap_model1_exp1 <- apa_print(model1_exp1)
#ap_model1_exp1
```

```{r}
model2_exp1 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
           , data = subset(ratings_and_parameters_long,us_valence%in%c("positive","negative")))
ap_model2_exp1 <- apa_print(model2_exp1)
#ap_model2_exp1
```
...
```{r}
ratings_and_parameters_long_8b <- merge(
   data_list_pilot$rating
   , get_theta(model = exp1_trait_model_8b, data = data_list_pilot$mpt_data_hierarchical)
 ) |>
   within({
...
contrasts(ratings_and_parameters_long_8b$us_valence) <- "contr.sum"
```

```{r}
model3_exp1 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d))*us_valence + (1|sid)
           , data = subset(ratings_and_parameters_long_8b,us_valence%in%c("positive","negative")))
ap_model3_exp1 <- apa_print(model3_exp1)
#ap_model3_exp1
```
...
```{r}
model4_exp1 <- lm(ec_effect ~ cntr(D)*cntr(C)*cntr(d),data = ratings_and_parameters_ec)
#summary(model1_exp1)
ap_model4_exp1 <- apa_print(model4_exp1)
#ap_model4_exp1
```

```{r}
model5_exp1 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
           , data = subset(ratings_and_parameters_ec_long,us_valence%in%c("positive","negative")))
...
#### wsw8b with EC
```{r}
ratings_and_parameters_long_8b_ec <- merge(
   data_list_pilot$rating
   , get_theta(model = exp1_trait_model_8b_ec, data = data_list_pilot$mpt_data_hierarchical)
 ) |>
   within({
     d <- ifelse(us_valence == "positive", d_positive, d_negative)
     C <- ifelse(us_valence == "positive", C_positive, C_negative)
...
contrasts(ratings_and_parameters_long_8b_ec$us_valence) <- "contr.sum"
```

```{r}
model6_exp1 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence + (1|sid)
           , data = subset(ratings_and_parameters_long_8b_ec,us_valence%in%c("positive","negative")))
ap_model6_exp1 <- apa_print(model6_exp1)
#ap_model6_exp1
```
...
```{r}
# individual_parameters    <- get_theta(exp2_trait_model   , data = data_list$mpt_data_hierarchical)
# individual_parameters_8b <- get_theta(exp2_trait_model_8b, data = data_list$mpt_data_hierarchical)
# 
# ratings_and_parameters <- merge(data_list$rating_wide, individual_parameters)
# 
# ratings_and_parameters_long_8b <- merge(data_list$rating, individual_parameters_8b) |>
#   within({
#     d <- ifelse(us_valence == "positive", d_positive, d_negative)
#     C <- ifelse(us_valence == "positive", C_positive, C_negative)
#   })
# 
# ratings_and_parameters_long <- merge(data_list$rating, individual_parameters)
# 
# ratings_and_parameters_long <- subset(ratings_and_parameters_long,us_valence %in% c("positive","negative"))
# 
# ratings_and_parameters_long_8b <- subset(ratings_and_parameters_long_8b,us_valence %in% c("positive","negative"))
# 
# contrasts(ratings_and_parameters_long$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long$task_order) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b$task_order) <- "contr.sum"
```



```{r}
# model_exp2 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence*task_order + (1|sid)
#            , data = ratings_and_parameters_long_8b)
# ap_model_exp2 <- apa_print(model_exp2)
#ap_model_exp2
```
...
#   , get_theta(exp3_trait_model, data = data_list$mpt_data_hierarchical)
# )
# lm(formula = ec_effect ~ cntr(D) * cntr(C) * cntr(d), data = ratings_and_parameters) |>
#   summary()

# ratings_and_parameters_pilot <- merge(
#   data_list_pilot$rating_wide
#   , get_theta(exp3pilot_trait_model, data = data_list_pilot$mpt_data_hierarchical)
# )
# lm(formula = ec_effect ~ cntr(D) * cntr(C) * cntr(d) + task_focus, data = ratings_and_parameters_pilot) |>
#   summary()

# ratings_and_parameters_joint <- merge(
#   data_list_joint$rating_wide
#   , get_theta(exp3joint_trait_model, data = data_list_joint$mpt_data_hierarchical)
# )

# ratings_and_parameters_pilot$sid <- paste0(ratings_and_parameters_pilot$sid,"pilot")
# 
# ratings_and_parameters_long_8b <- merge(
#   data_list$rating
#   , get_theta(exp3_trait_model_8b, data = data_list$mpt_data_hierarchical)
# ) |>
#   within({
#     d <- ifelse(us_valence == "positive", d_positive, d_negative)
#     C <- ifelse(us_valence == "positive", C_positive, C_negative)
#   })
# 
# ratings_and_parameters_long_8b_pilot <- merge(
#   data_list_pilot$rating
#   , get_theta(exp3pilot_trait_model_8b, data_list_pilot$mpt_data_hierarchical)
# ) |>
#   within({
#     d <- ifelse(us_valence == "positive", d_positive, d_negative)
#     C <- ifelse(us_valence == "positive", C_positive, C_negative)
#   })
# 
# ratings_and_parameters_long_8b_pilot$sid <- paste0(ratings_and_parameters_long_8b_pilot$sid,"pilot")
# 
# ratings_and_parameters_long <- merge(
#   data_list$rating
#   , get_theta(exp3_trait_model, data = data_list$mpt_data_hierarchical)
...
# ratings_and_parameters_long_pilot <- merge(
#   data_list_pilot$rating
#   , get_theta(exp3pilot_trait_model, data = data_list_pilot$mpt_data_hierarchical)
# )
# 
# ratings_and_parameters_long_pilot$sid <- paste0(ratings_and_parameters_long_pilot$sid,"pilot")
# 
# ratings_and_parameters_long <- subset(ratings_and_parameters_long,us_valence %in% c("positive","negative"))
# 
# ratings_and_parameters_long_8b <- subset(ratings_and_parameters_long_8b,us_valence %in% c("positive","negative"))
# 
# ratings_and_parameters_long_pilot <- subset(ratings_and_parameters_long_pilot,us_valence %in% c("positive","negative"))
# 
# ratings_and_parameters_long_8b_pilot <- subset(ratings_and_parameters_long_8b_pilot,us_valence %in% c("positive","negative"))
# 
# contrasts(ratings_and_parameters_long$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long$task_focus) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b$task_focus) <- "contr.sum"
# 
# contrasts(ratings_and_parameters_long_pilot$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b_pilot$us_valence) <- "contr.sum"
# contrasts(ratings_and_parameters_long_pilot$task_focus) <- "contr.sum"
# contrasts(ratings_and_parameters_long_8b_pilot$task_focus) <- "contr.sum"
# 
# ratings_and_parameters_both <- rbind(ratings_and_parameters,ratings_and_parameters_pilot)
# #colnames(ratings_and_parameters)
# #colnames(ratings_and_parameters_pilot)
# 
# ratings_and_parameters_long_both <- rbind(ratings_and_parameters_long,ratings_and_parameters_long_pilot)
# 
# ratings_and_parameters_long_8b_both <- rbind(ratings_and_parameters_long_8b,ratings_and_parameters_long_8b_pilot)
# 
```

```{r}
# ratings_and_parameters_long_8b_both$study <- factor(ratings_and_parameters_long_8b_both$study)
# 
# contrasts(ratings_and_parameters_long_8b_both$study) <- "contr.sum"
# 
# model_exp3 <- afex::lmer(evaluative_rating ~ (cntr(D)+cntr(C)+cntr(d)+cntr(b)+cntr(a))*us_valence*task_focus*study + (1|sid)
#            , data = ratings_and_parameters_long_8b_both)
