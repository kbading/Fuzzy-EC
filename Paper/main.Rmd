...
Despite their many differences, most of these accounts share a core idea: EC involves retrieving some information about the US upon evaluating the CSs, whether this memory is conscious or not. This assumption is well-supported by meta-analytical evidence showing that CS-US pairing memory is a strong moderator of EC [@hofmann_evaluative_2010] (see also, e.g., Pleyers et al., 2007). 
...
Experiment 1 was conducted to explore whether the MPT model adapted from @klauer_who_1998 can be used to measure CS-US pairing memory in the context of a standard EC procedure.
...
We recruited 50 participants (xxx$50\%$ female; $M_{age} = 38.68$; $SD_{age} = 15.01$).
...
For the MPT model analyses, the responses from the memory task were first recoded into a joint memory index according to the following scheme:
...
legend(
  x = .02
  , y = 5.98
  , legend = c("Rating first", "Memory first")
  , pch = 21
  , col = "black"
  , pt.bg = 1:2
  , bty = "n"
...
We again tested the relationships between individual-level parameter estimates and EC effects by calculating Bayes Factors for the slopes in the linear-regression part of the joint model. We found very strong support for a relationship between US valence memry (parameter $d$) and EC,
`r BFs$d`.
For all other MPT parameters, the Bayes factors quantifying evidence for similar relationships with EC effects were inconclusive, all `r BF_summary`.
