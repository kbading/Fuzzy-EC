...
This seminal distinction between memory for the US identity and memory for the US valence is important in EC research. Conceptually, US valence memory is less specific than US identity memory, making it the more suitable construct for testing the memory underpinnings of EC (Hütter et al., 2012; Stahl et al., 2009; Shanks and St. John, 1994). Theoretically, the distinction between US identity and US valence memory is crucial because different accounts of EC draw on different types of pairing memory as its cognitive foundation (e.g., Walther, Gawronski, Blank, & Langer, 2009; Gast & Rothermund, 2011). As a consequence, conceptualizing US identity and US valence memory as distinct constructs that can be measured separately is an important step towards a better understanding of EC. 

The present study aims to introduce a measurement model that can provide more informative estimates of CS-US memory (than those obtained in previous studies) and new insights into the relationships between CS-US pairing memory and EC as a result. In the remainder of the introduction, we briefly present common practices in assessing CS-US pairing memory and discuss four important shortcomings of the typical US identity and US valence memory measures. We then introduce a multinomial processing tree (MPT) model that solves these shortcomings and is, as a result, well positioned to provide new and arguably improved estimates of US valence and US identity memory, among other estimates that it allows. 
...
In a typical EC procedure for measuring CS-US pairing memory, the CS-US pairing memory task is administered after the evaluation task or in the same block. 
To measure US identity memory, each of the CS presented in the learning phase is displayed again, together with a fixed number of USs including the correct one. The presented USs may all share the same valence (identical to that of the previously paired US; e.g., Stahl & Bading, 2020) or be partly positive and partly negative (typically in equal numbers; e.g., Gast & Rothermund, 2011). 
...
For instance, Hütter et al. (2012) reasoned that standard measures of US valence memory may be affected by a phenomenon called affect-as-information.
Affect-as-information happens when participants rely on their affective reaction towards the CS to select a response in the US valence memory measure, possibly in the absence of any CS-US pairing memory.
...
In the next section, we will add to this critique by showing that affect-as-information (as a source of responding in measures of US identity and US valence memory) also interferes with separating the respective contributions of different types of pairing memory in the emergence of EC. While we only briefly touch on affect-as-information in the introduction, we will return to this point in more detail in the general discussion
...
A first shortcoming in current CS-US pairing memory measures is that US valence is perfectly inferable from US identity. When participants recall the specific identity of USs paired with CSs, US valence memory is not independent from US identity memory. 
...
## Shortcoming 2: US valence may be used to guess US identity
...
This implies that common measures of pairing memory may not deliver strong conclusions (about the respective contributions of US identity and US valence memory) when they reflect the true state of affairs.

## Shortcoming 3: US identity and US valence measures may be biased by response tendencies
...
## Shortcoming 4: US identity and US valence measures may confound CS-US pairing and CS recognition memory
...
# A solution to the identified measurement shortcomings: A multinomial processing tree model for memory involved in evaluative conditioning
...
To test the suitability of the MPT model we introduced as a solution to four shortcomings of common CS-US pairing memory measures on the one hand, and provide new insights into the relationships between EC, US valence memory, and US identity memory on the other hand, we report three experiments.
...
In the test phase, participants performed two measurement tasks: a memory task followed by a CS evaluation task. 
...
After pressing the spacebar, particpants worked through the CS evaluation task, consisting of 48 trials.
In the CS evaluation task, the 24 CSs and the 24 distractor stimuli were displayed individually without time limit. 
...
apa_table(apa_wsw6,caption="Experiment 1: Parameter estimates and 95% confidence intervals from the unrestricted 5-parameter model.")
```
...
<!-- The Bayes Factors quantifying evidence for or against relationships between MPT parameters and EC were inconclusive, all `r BF_summary`. -->
<!-- If you want more detail, we can of course also provide standardized regression coefficients and Bayes factors. -->
<!-- For instance, for parameter D, the standardized regression coefficent for $D$ was -->
<!-- `r apa_model_lm$estimate$D`, -->
<!-- and the corresponding Bayes Factor was -->
<!-- `r apa_model_lm$statistic$D`. -->
<!-- If you want to report both together, you can use -->
<!-- `r apa_model_lm$full_result$D`. -->

## Discussion
In Experiment 1, we used the MPT model to estimate EC-related memory in the context of a standard EC procedure.
We found that responses from the two-step memory task were well described by the 5-parameter model (including joint $D$, $C$, $d$, $a$ and $b$ parameters).
...
In addition, we collected a larger number of participants (to achieve more precise estimates of the mean EC effect and of the relationships between individual EC effects and MPT parameter estimates).
...
In the "evaluation first" condition, participants performed the evaluation task and then the memory task.
The task order was reversed in the "memory first" condition.
...
However, to simplify the results sections, the results of these analyses are reported in an online supplement (see OSF repository).
...
(ref:exp2-TBmodel-label) Relationships between individual-level MPT model parameters and marginal EC effects in Experiment 1. Points represent posterior means, error bars represent 95% credible intervals. Lines represent linear regression functions, dashed lines represent corresponding 95% credible bands.
...
We again tested the relationships between participant-level parameter estimates and EC effects by calculating Bayes Factors for the slopes in the linear-regression part of the joint model.
