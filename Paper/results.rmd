...
model.new <- lmer(
  formula = rating.bc ~ cntr(d) + cntr(Cpn) + us_valence + order + us_valence:order + cntr(d):us_valence + cntr(Cpn):us_valence + cntr(d):us_valence:order + cntr(Cpn):us_valence:order + cntr(d):cntr(Cpn):us_valence + cntr(d):cntr(Cpn):us_valence:order
  + (1 + us_valence| url.srid)
  , data = rating.ec
)
summary(model.new)

ggpredict(model.new, terms = ~ Cpn*us_valence*d) |>
  plot(rawdata = FALSE)

ggpredict(model.new, terms = ~ Cpn*us_valence*order) |>
  plot(rawdata = FALSE)
