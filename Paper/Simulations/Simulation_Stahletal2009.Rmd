...
# model_all <- afex::lmer(formula = CS_rating ~ US_valence * valmem_correct * idmem_mean_cntr + (0| sid)
#      , data = sim_data
#      )

# 'Initial analysis'
agg <- aggregate(cbind(avg_rating = CS_rating) ~ sid, data = sim_data, FUN = mean)

sim_data <- merge(sim_data, agg)

res_model <- lm(CS_rating ~ avg_rating, data = sim_data)
sim_data$init_residuals <- unname(residuals(res_model) * ifelse(sim_data$US_valence == "positive", 1, -1))

# Analysis proper
model_all <- lm(
  formula = init_residuals ~ valmem_correct * idmem_mean_cntr
  , data = sim_data
)
