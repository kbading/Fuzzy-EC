...
CS_name <- rep(CS_names,n_participants)
...
sim_data$valmem_correct <- ifelse(sim_data$idmem_prop > 0,1,rbinom(1,1,0.5))
...
model_all <- afex::lmer(formula = CS_rating ~ US_valence * (valmem_correct + idmem_mean_cntr) + (0 + US_valence| sid)
     , data = sim_data
     )
...
```{r}
ggeffects::ggeffect(model_all, terms = ~ valmem_correct:US_valence) |> plot()
```

```{r}
ggeffects::ggeffect(model_all, terms = ~ idmem_mean_cntr:US_valence) |> plot()
```

```{r}
ggeffects::ggeffect(model_all, terms = ~ idmem_mean_cntr:US_valence:valmem_correct) |> plot()
```

```{r}
ggeffects::ggeffect(model_all, terms = ~ valmem_correct:US_valence:idmem_mean_cntr) |> plot()
```
