...
(ref:intro-model1-label) The MPT model trees for positively and negatively paired CSs. CS = Conditioned Stimulus; US = Unconditioned Stimulus. The boxes on the left represent the stimuli displayed in the memory task, and the boxes on the right represent the responses. $D_{pos}$ = probability of recognizing a CS presented with a positive US as coming from the learning phase; $D_{neg}$ = probability of recognizing a CS presented with a negative US as coming from the learning phase; $C_{pos}$ = probability of retrieving the identity of the positive US; $C_{neg}$ = probability of retrieving the identity of the negative US; $d_{pos}$ = probability of retrieving the valence of the positive US; $d_{neg}$ = probability of retrieving the valence of the negative US; $b_{pos}$ = probability of guessing that a CS paired with a positive US is old; $b_{neg}$ = probability of guessing that a CS paired with a negative US is old; $a_{pos}$ = probability of guessing that a CS paired with a positive US was paired with a positive US; $a_{neg}$ = probability of guessing that a CS paired with a negative US was paired with a positive US; $g_{pos}$ = probability of correctly guessing the identity of the positive US a CS was paired with; $g_{neg}$ = probability of correctly guessing the identity of the negative US a CS was paired with.

(ref:intro-model2-label) The MPT model tree for unpaired distractor stimuli. $D_{new}$ = probability of detecting a new stimulus as new; $b_{new}$ = probability of guessing that a new stimulus is old; $a_{new}$ = probability of guessing that a new stimulus was paired with a positive US.

```{r illustrate-mpt-1, fig.cap = "(ref:intro-model1-label)", out.width="100%"}
...
```{r illustrate-mpt-2, fig.cap = "(ref:intro-model2-label)", out.width="100%"}
...
### Conclusion
The present research introduced a multinomial processing tree model to provide a more precise account of the memory representations underlying evaluative conditioning. Across three experiments using different materials and procedures, the model produced coherent and theoretically meaningful parameter estimates. Crucially, the results indicate that memory for US valence constitutes a dissociable form of contingency memory that predicts evaluative change, whereas US identity memory---although robustly encoded---did not reliably account for EC effects under the present conditions. These findings support the view that abstracted evaluative information can guide attitudes even when memory for specific pairings is unavailable.

More broadly, the present work highlights the importance of measurement precision in research on evaluative learning. By separating CS recognition, identity memory, valence memory, and guessing processes within a unified framework, the proposed MPT approach reduces key inferential ambiguities that characterize standard memory measures and enables more balanced tests of competing theoretical claims. Future research can build on this framework to examine how learning conditions, task demands, and retention intervals shape the relative contribution of identity-based and valence-based representations, thereby further clarifying the mechanisms through which experience translates into evaluative change.
