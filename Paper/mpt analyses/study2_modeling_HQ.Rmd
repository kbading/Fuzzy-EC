...
xxx.1 <- subset(dat,val_us_accuracy == "val_correct" & id_us_accuracy == "id_correct")

xxx.2 <- subset(dat,val_us_accuracy == "val_correct" & id_us_accuracy == "id_incorrect")

xxx.3 <- subset(dat,val_us_accuracy == "val_correct")

xxx.4 <- subset(dat,val_us_accuracy %in% c("val_correct","val_incorrect"))

aov_ez(xxx.4,dv = "cs_rating", id = "url.srid", within = c("us_valence","val_us_accuracy"))

aov_ez(xxx.3,dv = "cs_rating", id = "url.srid", within = c("us_valence","id_us_accuracy"))

aov_ez(xxx.1,dv = "cs_rating", id = "url.srid", within = c("us_valence"))
aov_ez(xxx.2,dv = "cs_rating", id = "url.srid", within = c("us_valence"))
```
...
if(file.exists("study2_bothPNd_HQ.rds")) {
  study2_bothPNd_HQ <- readRDS("study2_bothPNd_HQ.rds")
} else {

  study2_bothPNd_HQ = traitMPT(
  eqnfile = "KW_wsw_model.eqn"
  , data = mpt_data
  , n.iter   = 4e5
  , n.adapt  = 2e4
  ...
  , n.thin   = 4e1
  , n.chains = 4
  , restrictions = list("Dn=Dpos=Dneg","G=0.25","dpos=dneg=0")
  , ppp = 2000#5e3
  , covData=cov_data
  , predStructure=list("a; order","b; order","Dn; order","Cpos; order","Cneg; order"
                       )
  , IVprec = "dgamma(.5,.5)"
  #, predType = c("f")
  )
  saveRDS(study2_bothPNd_HQ, file = "study2_bothPNd_HQ.rds")
}
...
individual_parameters <- as.data.frame(getParam(study2_bothPNd_HQ, parameter = "theta"))
parameter_names <- colnames(individual_parameters)
individual_parameters$id <- rownames(individual_parameters)
#saveRDS(individual_parameters, file = "individual-parameters-KB_both.rds")


para_ratings_bothPNd<- merge(mpt_data_subset, individual_parameters, by = "id")
save(para_ratings_bothPNd,file = "HQstudy2bothPNd_x_ratings.rdata")
```
...
model_files <- list.files(path = file.path(study_folder), pattern = "^study2_bothPNd", full.names = TRUE)
...
names(waic) <- basename(model_files) |>
  gsub(pattern = "^study2_|\\.rds$", replacement = "") |>
  gsub(pattern = "-", replacement = "_")

saveRDS(waic, file = file.path(study_folder, "waicPNd.rds"))
