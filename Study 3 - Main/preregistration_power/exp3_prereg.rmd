...
To test prediction P1 ($EC_\textrm{valence-focus}>EC_\textrm{age-focus}$), we will conduct a two-sample *t* test comparing EC effects between task focus conditions.
...
Note that the one-sided *t* test ($\alpha=.1$) is an analogue of the *F* test ($\alpha=.1$) for the two-way interaction of US valence and task focus (which was used to determine the mimimal sample size).
...
To assess model adequacy (i.e., model fit),
we will calculate the posterior-predictive fit statistic $T_1$ [@klauer_hierarchical_2010] together
with its corresponding posterior-predictive $p$ value.
...
we will estimate separate models (each of which will include one of 12 restrictions:
...
we will again estimate separate models (each of which will include one of six restrictions:
[13] $D_\textrm{valence-focus}=D_\textrm{age-focus}$,
[14] $C_\textrm{valence-focus}=C_\textrm{age-focus}$,
[15] $d_\textrm{valence-focus}=d_\textrm{age-focus}$,
[16] $b_\textrm{valence-focus}=b_\textrm{age-focus}$,
[17] $a_\textrm{valence-focus}=a_\textrm{age-focus}$, 
[18] $g_\textrm{valence-focus}=g_\textrm{age-focus}$)
