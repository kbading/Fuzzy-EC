...
date              : "`r Sys.Date()`"
bibliography      : "`r source(file.path(rprojroot::find_rstudio_root_file(), '.Rprofile')); methexp_bib()`"
author: 
...
The MPT model is an adaptation of the "who said what" model [@klauer_unraveling_1998] to the evaluative conditioning paradigm and estimates memory retrieval of conditioned and unconditioned stimuli at different levels of specificity.
...
We will use these default priors for both the baseline model and all restricted variants of the model (see below).
