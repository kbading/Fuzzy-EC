---
title: On the possible role of attitude-based guessing in the Who-said-what? model of evaluative conditioning 
author: mariusbarth
date: today
code-fold: true
editor_options: 
...
simulate_participant <- function(
  ...
  if(rating == "binary-attitude-based") {
    evaluative_rating <- ifelse(tau_attitude > 0, "positive", "negative") |> factor(levels = c("positive", "negative"))
    ...
    evaluative_rating[!idx] <- ifelse(tau_attitude[!idx] > 0, "positive", "negative") |> factor(levels = c("positive", "negative"))
...
options(width = 250, contrasts = c("contr.sum", "contr.poly"))
...
Does consistent responding between assignment task and evaluative ratings inflate $d$s
...
d$evaluative_rating <- factor(d$evaluative_rating)
...
controlling $a$ might result in an over-correction (i.e., deflation of $d$).
...
If US valence memory affects evaluative ratings,



# Simulation 8

Same as Simulation\ 7, but attitudes are not conditioned, but only pre-existing
(i.e., not correlated with US valence).

This scenario implements the idea that evaluative rating are affected by attitudes,
but there is no such thing as conditioned attitude.
