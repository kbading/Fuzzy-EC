...
The present study aims to introduce a measurement model that can provide more informative estimates of CS-US memory (than those obtained in previous studies) and new insights into the relationships between CS-US pairing memory and EC as a result. 
In the remainder of the introduction, we briefly present common practices in assessing CS-US pairing memory and discuss four important shortcomings of the typical measures of US identity and US valence memory.
We then introduce a multinomial processing tree (MPT) model that solves these shortcomings and is, as a result, well positioned to provide new and arguably improved estimates of US valence and US identity memory, among other estimates that it allows. 
...
In a typical procedure for measuring CS-US pairing memory, the memory task is administered after the CS evaluation task or in the same block. 
...
In the next section, we will add to this critique by showing that, when combined with another shortcoming of standard US valence measures, affect-as-information also interferes with demonstrating a statistical relationship between EC effect and US identity memory, thereby precluding positive evidence for S-S models of EC (e.g., Baeyens, Eelen, Van den Bergh, & Crombez, 1992; Hammerl & Grabitz, 1996; Walther, Gawronski, Blank, & Langer, 2009).
...
```{r}
# par(mfrow=c(1,1))
# apa_beeplot(data= exp2_ratings_wide,id = "sid", dv = "ec_effect", factors = c("task_order"))
```
...
By contrast, the significant $d$ parameter (from Experiment 2) was not replicated by the present experiment.
