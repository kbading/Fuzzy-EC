...
## Hierarchical MPT model w/ EC effect as predictor
...
contrasts(data$task_focus) <- "contr.sum"                   # = effects coding (i.e. sum-to-zero contrasts)
cntr <- center <- function(x) { x - mean(x, na.rm = TRUE) } # = function for centering

model <- lm(
  formula = ec_effect ~ (cntr(D) + cntr(C) + cntr(d) + cntr(a) + cntr(b))*task_focus
  , data = data
)
...
summary(model)
plot(model)
#ggpredict(model, terms = ~ D + task_focus) |>
  #plot(show_data = TRUE)

library(emmeans)

emmeans(model, specs = ~ task_focus) |> 
...
```{r }
ggeffect(model, terms = ~ task_focus) |>
  plot(show_data = TRUE)
```

```{r }
ggeffect(model, terms = ~ cntr(D) * task_focus) |>
  plot(show_data = TRUE)
```

```{r }
ggeffect(model, terms = ~ cntr(C) * task_focus) |>
  plot(show_data = TRUE)
```
...
mediation_syntax <-"
    ...
    ab_dC := a_dC*b_dC
    ab_DdC := a_DdC*b_DdC
    c := ade + ab_D + ab_C + ab_d + ab_b + ab_a + ab_Dd + ab_DC + ab_dC + ab_DdC  #having defined ab, we can use this here.
"
...
```{r}
hist(log(data$p_us_identity))
hist(log(data$p_us_valence))
data$ln_us_identity <- log(data$p_us_identity)
data$ln_us_valence <- log(data$p_us_valence)
...
mediation_syntax <-"
    ...
    ec_effect ~ b_D*D + b_C*C + b_d*d + b_b*b + b_a*a + b_us_valence*ln_us_valence + b_us_identity*ln_us_identity + ade*task_focus
    ...
    c := ade + ab_D + ab_C + ab_d + ab_b + ab_a + ab_us_valence + ab_us_identity  #having defined ab, we can use this here.
"
