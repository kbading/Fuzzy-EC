...
```{r}
data_mem <- data_list$memory
table(data_mem$sid,data_mem$reco_resp)
data_mem$reco_resp_num <- ifelse(data_mem$reco_resp == "old",1,0)

data_mem_val <- subset(data_mem,task_focus=="valence")
table(data_mem_val$sid,data_mem_val$reco_resp)
data_mem_val_agg <- aggregate(data=data_mem_val,FUN=mean,reco_resp_num ~ sid)
quantiles_val <- quantile(data_mem_val_agg$reco_resp_num)
upborder_val <- quantiles_val[[3]] + 3 * (quantiles_val[[4]]-quantiles_val[[2]])
lowborder_val <- quantiles_val[[3]] - 3 * (quantiles_val[[4]]-quantiles_val[[2]])

data_mem_val_agg$outlier <- ifelse(data_mem_val_agg$reco_resp_num>upborder_val|data_mem_val_agg$reco_resp_num<lowborder_val,1,0)

data_mem_age <- subset(data_mem,task_focus=="age")
table(data_mem_age$sid,data_mem_age$reco_resp)
data_mem_age_agg <- aggregate(data=data_mem_age,FUN=mean,reco_resp_num ~ sid)
quantiles_age <- quantile(data_mem_age_agg$reco_resp_num)
upborder_age <- quantiles_age[[3]] + 3 * (quantiles_age[[4]]-quantiles_age[[2]])
lowborder_age <- quantiles_age[[3]] - 3 * (quantiles_age[[4]]-quantiles_age[[2]])

data_mem_age_agg$outlier <- ifelse(data_mem_age_agg$reco_resp_num>upborder_age|data_mem_age_agg$reco_resp_num<lowborder_age,1,0)

data_mem_age_agg <- subset(data_mem_age_agg,outlier == 0)
data_mem_val_agg <- subset(data_mem_val_agg,outlier == 0)

inlier.ids <- c(unique(data_mem_age_agg$sid),unique(data_mem_val_agg$sid))
```
...
```{r}
#data_mem_sub <- subset(data_mem,sid %in% inlier.ids)
data_mem_sub <- subset(data_mem,sid != 74 & sid != 51 & sid != 45)
data_mem_sub$identity <- 1L
mpt_dat <- as.data.frame(unclass(table(data_mem_sub$identity, data_mem_sub$mpt_response_condition)))
...
summary(model, pars = "beta") |> print(digits = 3)
