...
```{r}
mem_dat <- data$memory
table(mem_dat$sid,mem_dat$reco_resp,mem_dat$task_order)
```
...
models <- lapply(
  ...
  , FUN = function(x) {
    fit_mpt(
      ...
      add_condition("Task order" = c("Rating first" = "_x1", "Memory first" = "_x2")) |>
...
#ec_lm <- lm(formula = ec_effect ~ task_order, data = data)
...
emtrends(ec_lm, var = "C", specs = ~ task_order) |>
  summary(infer = TRUE)
