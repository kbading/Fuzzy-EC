...
pict = pict %>% filter(theme != "Nude" & theme_long != "Animal car" & theme != "Fece")

describeBy(pict$Valence_mean, pict$Category)

table(pict$Category, pict$Valence_mean < 4)

pict = pict %>% filter(Category == "Scene")
...
pict_neg = pict %>% filter(Valence_mean >= 2 & Valence_mean <= 3
                           & Arousal_mean >= 3 & Arousal_mean <= 5)
...
pict_pos = pict %>% filter(Valence_mean >= 5 & Valence_mean <= 6
                           & Arousal_mean >= 3 & Arousal_mean <= 5)
...
write.csv(selected_pictures, "selected_pictures_tbs_attrib.csv")
