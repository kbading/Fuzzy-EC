...
# vector containing all US names
correct_us <- unique(dat_count2$correct_us_source)

# to create a full crossover for each US with all 24 USs ...
# ... create vector with the same US name per 24 lines (for all USs)
correct_us_rep <- rep(correct_us,each = 24)
# ... duplicate vector "correct_us" 24 times
...
# how often is each US the correct option?
salut$count.baseline <- 0
# how often is US presented as an option for a given "correct US"?
salut$count.presented <- 0
# relative frequency of US option presented as an option for a given "correct US"?
...
salut.2$relative.n_selected <- salut.2$n_selected/salut.2$count.presented # use this for analysis
salut.2$conditional <- salut.2$relative.n_selected/salut.2$relative.count # kind of like an odds ratio? havent thought this through
...
#### calculate sel_val_cor (= selected valence correct) only for USs that were actually [incorrectly] chosen for a given correct US
salut.4$sel_val_cor <- ifelse(salut.4$n_selected > 0 & salut.4$valence_cor == salut.4$valence_selected
                                  ,1,ifelse(salut.4$n_selected > 0 & salut.4$valence_cor != salut.4$valence_selected
                                            ,0,NA))
table(salut.4$sel_val_cor)
apa_barplot(data = salut.4
            , main = ""
            , id = "correct_us_source"
            , dv = "sel_val_cor"
            , factors = c("valence_cor","Category_cor"))

salut.5 <- aggregate(data = salut.4, FUN = mean, sel_val_cor ~ correct_us_source + valence_cor + Category_cor)
aov_ez(data = salut.5
      , id = "correct_us_source"
      , dv = "sel_val_cor"
      , between = c("valence_cor","Category_cor"))

# --> 3 Category_cor x 2 valence_cor combinations seem to contribute pretty evenly to within-valence errors (see plot)
# --> no significant effects in ANOVA

### For each combination, are within-valence errors driven by match between Category_cor and Category_selected?

# positive animals
only.animal.pos.pos <- subset(salut.4,Category_cor == "Animal" & valence_cor == "positive" & count.presented > 0 & valence_selected == "positive")
apa_barplot(data = only.animal.pos.pos
            , main = "animal & positive & positive"
            , id = "correct_us_source"
            , dv = "relative.n_selected"
            , ylim = c(0,1)
            , factors = c("Category_selected"))
#--> positive animals not over-represented

## for reference
only.animal.pos.neg <- subset(salut.4,Category_cor == "Animal" & valence_cor == "positive" & count.presented > 0 & valence_selected == "negative")
...
OAP.agg <- aggregate(data = only.animal.pos, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OAP.agg.2 <- dcast(OAP.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OAP.agg.2$diff <- OAP.agg.2$`positive` - OAP.agg.2$`negative`
OAP.agg.2$id <- 1
...
# positive objects
only.object.pos.pos <- subset(salut.4,Category_cor == "Object" & valence_cor == "positive" & count.presented > 0 & valence_selected == "positive")
...
## for reference
only.object.pos.neg <- subset(salut.4,Category_cor == "Object" & valence_cor == "positive" & count.presented > 0 & valence_selected == "negative")
...
OOP.agg <- aggregate(data = only.object.pos, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OOP.agg.2 <- dcast(OOP.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OOP.agg.2$diff <- OOP.agg.2$`positive` - OOP.agg.2$`negative`
OOP.agg.2$id <- 1
...
# positive scenes
only.scene.pos.pos <- subset(salut.4,Category_cor == "Scene" & valence_cor == "positive" & count.presented > 0 & valence_selected == "positive")
...
## for reference
only.scene.pos.neg <- subset(salut.4,Category_cor == "Scene" & valence_cor == "positive" & count.presented > 0 & valence_selected == "negative")
...
OSP.agg <- aggregate(data = only.scene.pos, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OSP.agg.2 <- dcast(OSP.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OSP.agg.2$diff <- OSP.agg.2$`positive` - OSP.agg.2$`negative`
OSP.agg.2$id <- 1
...
# negative animals
only.animal.neg.neg <- subset(salut.4,Category_cor == "Animal" & valence_cor == "negative" & count.presented > 0 & valence_selected == "negative")
apa_barplot(data = only.animal.neg.neg
            ...
            , ylim = c(0,1)
            , factors = c("Category_selected"))
#--> negative animals slightly over-represented

## for reference
only.animal.neg.pos <- subset(salut.4,Category_cor == "Animal" & valence_cor == "negative" & count.presented > 0 & valence_selected == "positive")
...
OAN.agg <- aggregate(data = only.animal.neg, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OAN.agg.2 <- dcast(OAN.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OAN.agg.2$diff <- OAN.agg.2$`negative` - OAN.agg.2$`positive`
OAN.agg.2$id <- 1
...
# negative objects
only.object.neg.neg <- subset(salut.4,Category_cor == "Object" & valence_cor == "negative" & count.presented > 0 & valence_selected == "negative")
...
## for reference
only.object.neg.pos <- subset(salut.4,Category_cor == "Object" & valence_cor == "negative" & count.presented > 0 & valence_selected == "positive")
...
OON.agg <- aggregate(data = only.object.neg, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OON.agg.2 <- dcast(OON.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OON.agg.2$diff <- OON.agg.2$`negative` - OON.agg.2$`positive`
OON.agg.2$id <- 1
...
# negative objects
only.scene.neg.neg <- subset(salut.4,Category_cor == "Scene" & valence_cor == "negative" & count.presented > 0 & valence_selected == "negative")
...
## for reference
only.scene.neg.pos <- subset(salut.4,Category_cor == "Scene" & valence_cor == "negative" & count.presented > 0 & valence_selected == "positive")
...
OSN.agg <- aggregate(data = only.scene.neg, FUN = mean, relative.n_selected ~ valence_selected * Category_selected)
OSN.agg.2 <- dcast(OSN.agg, value.var = "relative.n_selected", Category_selected ~ valence_selected)
OSN.agg.2$diff <- OSN.agg.2$`negative` - OSN.agg.2$`positive`
OSN.agg.2$id <- 1
...
# for each correct US: sum up how often each category x valence combination was presented as on option (excluding correct US)
mon <- subset(salut.4,correct_us_source != source_mem_resp)
mon.dieu <- aggregate(mon, FUN = sum, count.presented ~ correct_us_source * Category_selected * valence_selected + valence_cor + Category_cor)
# aggregate across valence_cor x Category_cor x valence_selected x Category_selected
mon.dieu.2 <- aggregate(data = mon.dieu, FUN = sum, count.presented ~ valence_cor * Category_cor * valence_selected * Category_selected)
# how often did each correct US receive an incorrect response?
dios <- salut.4[,c("correct_us_source","count.baseline","valence_cor","Category_cor")]
dios.mio <- aggregate(data = dios, FUN = mean, count.baseline ~ correct_us_source + valence_cor + Category_cor)
# number of available spots (8-1) for each correct US across all trials with incorrect response
dios.mio$spots <- dios.mio$count.baseline * 7
# aggregate across valence_cor x Category_cor
dios.mio.2 <- aggregate(data = dios.mio, FUN = sum, spots ~ valence_cor * Category_cor)

OMG <- merge(mon.dieu.2,dios.mio.2, by= c("valence_cor","Category_cor"),all.x = TRUE)
OMG$relative <- OMG$count.presented/OMG$spots

ciao <- subset(OMG,valence_cor == "positive" & Category_cor == "Animal")
apa_barplot(data = ciao
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))

ciao.2 <- subset(OMG,valence_cor == "negative" & Category_cor == "Animal")
apa_barplot(data = ciao.2
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))

ciao.3 <- subset(OMG,valence_cor == "positive" & Category_cor == "Object")
apa_barplot(data = ciao.3
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))

ciao.4 <- subset(OMG,valence_cor == "negative" & Category_cor == "Object")
apa_barplot(data = ciao.4
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))

ciao.5 <- subset(OMG,valence_cor == "positive" & Category_cor == "Scene")
apa_barplot(data = ciao.5
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))

ciao.6 <- subset(OMG,valence_cor == "negative" & Category_cor == "Scene")
apa_barplot(data = ciao.6
            , id = "valence_cor"
            , dv = "relative"
            , factors = c("valence_selected","Category_selected"))
