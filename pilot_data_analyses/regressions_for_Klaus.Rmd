...
The correlations between the mean rating of "new" CSs and Cpos, Cneg, dpos and dneg are all rather small, suggesting that ratings of "new" CSs is not related to recollection of US and/or US valence for previously paired "old" CSs.
...
The positive weight for the interaction between Cneg and US valence suggests a positive relationship between Cneg and ratings for positively paired CSs, and a negative relationship between Cpos and ratings for negatively paired CSs.
...
cntr <- center <- function(x) { x - mean(x, na.rm = TRUE) } # = function for centering
model <- lm(
  formula = ec ~ (cntr(a) + cntr(b) + cntr(Dn) + cntr(Cpos) + cntr(Cneg) + cntr(dpos) + cntr(dneg)) + cntr(Cpos):cntr(dpos) + cntr(Cneg):cntr(dneg)
  , data = para_ratings
)
...
As indicated by the marginally significant intercept, the is a tendency for an EC effect.
...
ggpredict(model, terms = ~ dpos * Cpos) |>
  plot(rawdata = TRUE)

ggpredict(model, terms = ~ dneg * Cneg) |>
  plot(rawdata = TRUE)
...
## Regression analysis 3b: Predict baseline-corrected EC effects from MPT parameters
Individual baseline-corrected EC effects will be calculated by subtracting the baseline-corrected mean rating of negatively paired CSs from the baseline-corrected mean rating of positively paired CSs.
The individual baseline-corrected EC effects will then be predicted from individual MPT parameters (D [discrimination old vs. new], b [guessing "old"], a [guessing "positive US"], Cpos [recollection of paired US for positively paired CSs], Cneg [recollection of paired US for negatively paired CSs], dpos [recollection of paired US valence for positively paired CSs], dneg [recollection of paired US valence for negatively paired CSs]).

```{r}

para_ratings$ec <- para_ratings$positive_bc-para_ratings$negative_bc
contrasts(data$us_valence) <- "contr.sum"                   # = effects coding (i.e. sum-to-zero contrasts)
cntr <- center <- function(x) { x - mean(x, na.rm = TRUE) } # = function for centering
model <- lm(
