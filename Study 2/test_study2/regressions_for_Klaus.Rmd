...
paired <- subset(dat_ev, us_valence != "dist")
...
mod2_print = apa_print(mod2)

apa_barplot(data = paired
            , id = "url.srid"
            , dv = "eval_rating"
            , factors = c("order","us_valence"))
...
cov_data <- mpt_data_b[,c("url.srid","order")]
mpt_data <- mpt_data_b[,2:12]
...
if(file.exists("mod_mpt_KB_memfirst.rds")) {
  mod_mpt_KB_both <- readRDS("mod_mpt_KB_both.rds")
} else {

  mod_mpt_KB_memfirst = traitMPT(
  ...
  #saveRDS(mod_mpt_KB_both, file = "mod_mpt_KB_both.rds")
}
...
individual_parameters <- as.data.frame(getParam(mod_mpt_KB_both, parameter = "theta"))
...
save(para_ratings_memfirst,file = "MPTmemfirst_x_ratings")
```
...
model <- lmer(
  ...
  (cntr(b) + cntr(d) + cntr(C)) * us_valence + (1 | id)
  , data = data
)
...
ggpredict(model, terms = ~ cntr(d) * us_valence) |>
  plot(rawdata = TRUE)
...
```{r}
ggpredict(model, terms = ~ cntr(C) * us_valence) |>
  plot(rawdata = TRUE)

ggpredict(model, terms = ~ cntr(D) * us_valence) |>
  plot(rawdata = TRUE)

ggpredict(model, terms = ~ cntr(a) * us_valence) |>
  plot(rawdata = TRUE)
```
...
cntr <- center <- function(x) { x - mean(x, na.rm = TRUE) } # = function for centering
model <- lm(
  formula = ec.2 ~ (cntr(b) + cntr(Cpos) + cntr(dpos) + cntr(dneg))
...
ggpredict(model, terms = ~ dpos *order) |>
  plot(rawdata = TRUE)
