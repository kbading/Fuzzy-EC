...
read_file('data_test_study2/data_raw_test_first_run.txt')%>%
#... split it into lines ...
 str_split('\n') %>% first() %>%
#... filter empty rows ...
 discard(function(x) x == '') %>%
#... parse JSON into a data.frame
 map_dfr(fromJSON, flatten=T) -> dat.1

dat.1$study = 'first_run'

read_file('data_test_study2/data_raw_test_second_run.txt')%>%
#... split it into lines ...
 str_split('\n') %>% first() %>%
#... filter empty rows ...
 discard(function(x) x == '') %>%
#... parse JSON into a data.frame
 map_dfr(fromJSON, flatten=T) -> dat.2

dat.2$study = 'second_run'

dat = full_join(dat.1, dat.2)
```
...
saveRDS(dat_final, "data_test_study2/data_wsw2.RDS") #RDS can handle lists such as in the `uss` list
```
